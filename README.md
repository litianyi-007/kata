...
| **Phase 1+** *(in flight)* | **Spec History Management (v1.13).** SDD / superpowers-style flows accumulate specs that drift apart over time. `wiki-spec preflight` makes new specs answer for the prior specs they overlap. Phase 0 ships in v2.2.0 (advisory). Phase 1-4 add external-source backfill, enforcement, auto-propagation, lineage view. |
