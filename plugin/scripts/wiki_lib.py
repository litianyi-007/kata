...
# --- Structural / meta files exempt from wikilink-graph treatment ---------
#
# graph_query.py --mode orphans previously counted SCHEMA.md, index.md, and
# log.md as "true orphans" on every single wiki (they never receive an
# inbound [[wikilink]] from a content page — nothing is supposed to link
# to a bookkeeping file). Worse, log.md commonly *describes* the wikilink
# syntax with a literal example — e.g. a log entry that reads "Cross-
# references: 38 [[wikilink]]" or a header like "Format: ## [YYYY-MM-DD]
# action | subject" — and extract_links() has no way to distinguish that
# from a real link, so it got resolved as an unresolvable target and
# reported as a dangling link (`dangling_links: {'log.md': ['wikilink']}`,
# reproduced against a real wiki, not just a synthetic fixture).
#
# Fix: these three top-level bookkeeping files are not "content pages" the
# wikilink graph is meant to describe at all, so (a) discover_pages() never
# runs extract_links() on their body (no out_links parsed → no false
# dangling-link entries can be produced from their prose), and (b)
# graph_query.py's orphans mode excludes them from true_orphans/leaves via
# is_structural_page() below.
#
# dreaming/*.md files (auto-generated by wiki_dream.py) get the *same*
# orphan-detection exemption (a candidate-less dreaming run has zero real
# graph edges and would otherwise be misreported as an orphan too) but
# deliberately keep normal wikilink-body parsing — unlike log.md, a
# dreaming digest's `[[candidate-page]]` references are real citations to
# real pages and must still resolve/build graph edges.
#
# raw/ and _archive/ need no entry here: discover_pages()'s skip_dirs
# already excludes them from the walk entirely, so nothing under them ever
# becomes a Page object in the first place — checked, not reproducible.
STRUCTURAL_FILENAMES = frozenset({"SCHEMA.md", "index.md", "log.md"})
STRUCTURAL_DIR_PREFIXES = ("dreaming/",)


def is_structural_page(path: str) -> bool:
    """True for wiki bookkeeping / meta-report files that are exempt from
    orphan detection (graph_query.py --mode orphans). `path` is the
    wiki-root-relative posix path (Page.path)."""
    return path in STRUCTURAL_FILENAMES or any(
        path.startswith(prefix) for prefix in STRUCTURAL_DIR_PREFIXES)
...
def discover_pages(wiki_root: Path) -> list[Page]:
    ...
    for root, dirs, files in os.walk(wiki_root):
        ...
        for fname in files:
            ...
            # SCHEMA.md / index.md / log.md are bookkeeping, not content —
            # their prose is never meant to hold real [[wikilinks]] (index.md
            # uses ordinary markdown `[text](path)` links; log.md's own
            # format header/entries can literally *mention* the `[[..]]`
            # syntax as an example). Skip extract_links() for them entirely
            # so that example text can never be misread as a dangling link
            # (see STRUCTURAL_FILENAMES above). dreaming/*.md is NOT in this
            # set — its candidate citations are real links and must still
            # resolve normally.
            out_links = [] if rel in STRUCTURAL_FILENAMES else extract_links(body)
            pages.append(Page(
                path=rel,
                title=str(title),
                frontmatter=fm,
