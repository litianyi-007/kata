...
NECallKit 是一个真实的 Electron + 原生视频通话 SDK——跨平台、原生 IPC bridge（不同进程之间互相调用的通道）、mac/win/linux、Java + JS + C++ 三种代码并存。**不是 toy fixture**。
...
整个 4 周里，Claude Code 和 Codex CLI 轮班做维护 agent，agent 总共跑了 **160+ 小时**的 session 时间。我自己 hands-on 大约 **15 小时**。**人和 agent 大约 1:11**。
...
三个不同的 session、三个不同时间点的 agent、三周间隔。**没有一个 session 把它当作一类来看待**。这个"类"在 chat 受限的智能里**结构上看不见**——每次 bug fix 都是局部的，pattern detection 需要"站在对话外面"。

**每次 session 都把代码改得严格更好，那个类却始终隐形。**

![L1 / L2 失败分类法——三个 bug 让"这一类"显形](../assets/essay/V2-l1-l2-failure-taxonomy.svg)
...
![t3：一条回填查询，+17 条边。导入平均一页 5 条。](../assets/essay/V1-wiki-compounding.svg)
...
这是 AI 工具**从代码内部修不动**的失败模式——缺失的规约不在语言里、不在框架里、不在任何公开 API 里。它在你团队的脑子里。

经济预测有点令人不舒服但很直白：当代码质量工作不断落到 AI 手里，资深工程师的可持续贡献就**从"产出代码"迁移到"产出代码必须遵守的规则"**——从"做出制品"变成"产出制品必须遵守的规约"。

wiki 是这种编译的一种形态。还会有其它形态——schema 仓库、contract test 套件、决策日志、机器可读的不变量声明。**任选一种。或者自己造一种。**

AI 在规约写下来的地方非常强。bug 是我们**从来没把那个不属于语言特性的部分写下来**，因为过去 15 年里没有这么做的动力。

**现在有了。**

**重点不是 wiki，是有任何一层"被编译过的业务规约"都好。**

最小的下一步：找你团队里被反复问的那一个问题，让你的 AI 把答案写下来、互链相关代码。这就是第一页。

> *（"这不就是 runbook / Confluence / FAQ 吗？"——区别在于"编译"。runbook 是步骤序列；这是图。Confluence 是终点页；这是其它页面会链入的 hub。FAQ 给一个答案；这在答案变了的时候会自己改。回填查询留下的 cross-link，下一个 session 在动代码之前就会读到。这是 runbook 不会做的事。）*

## 接下来

**老实承认的空白。** 我没有跑冷基线对比——同一个问题，同一个 agent，**不带 wiki** 的版本。上面那些数字说的是复利的**形状**，不是 wiki vs 没 wiki 的**幅度**。幅度是下一个实验。这个空缺已经在 dogfood 日志里标记好了（`dogfood-necallkit-hn-essay.md:996-999`）。

**范围声明。** 这篇文章讲的是其中一个用例——项目记忆。Kata 设计上还有其它部分（auto-dreaming、团队 spec 制定）但跟本文论点无关，感兴趣自己去仓库看。

**下一个代码实验。** v1.11 `wiki-session-ingest`（PRD Draft v2: `docs/PRD-v1.11-session-ingest.md`）——读当前 CLI session、抽取知识点、让用户多选哪些回填进 wiki 的一个 skill。它是对**另一类**失败模式（知识在对话里诞生，在 session 边界处死亡）的结构性回应，那是下一篇文章的主题。
...
你不必非得用 Kata 来做这件事。给你团队找任何一层"被编译过的业务规约"——fork 我的、写你自己的、或者搭一个完全不同的东西。

> *—— 工作日志，2026-05-13。下一个 slot：冷基线对照。*
