...
| `wiki-ingest` | `<url\|file\|text> [--batch] [--no-discuss] [--no-images] [--no-spec-preflight] [--set=key=value,...] [--page-type=<type>] [--proposed-path=<wiki-relative-path>] [--evidence-anchors=<comma-separated>]` | Ingest a source into the wiki: save raw content and referenced images, prompt the user for any cust… |
